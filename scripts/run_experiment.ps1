...
# ── Python interpreter: prefer the repo .venv if present ─────────────────────
# The .venv already has Flask + pytest installed. We just need to ensure
# requests is also available for collect_results.py.
...
# Ensure requests is installed (silently, only installs if missing)
& $PYTHON -m pip install requests --quiet --disable-pip-version-check 2>$null
if ($LASTEXITCODE -ne 0) {
    Write-Host "WARNING: Could not install requests. Collection may fail."
}
