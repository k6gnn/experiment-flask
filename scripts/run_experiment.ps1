...
#          .\scripts\run_experiment.ps1 E8 1 flaky_flask github
param(
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "WARNING: Baseline restore had errors - check manually before continuing."
}
...
if ($Platform -eq "github") {
    ...
    if ($LASTEXITCODE -ne 0) { Write-Host "ERROR: Push to GitHub failed. Aborting."; exit 1 }
    Write-Host "  Pushed to GitHub (origin/main)"
}
elseif ($Platform -eq "gitlab") {
    git push gitlab main
    if ($LASTEXITCODE -ne 0) { Write-Host "ERROR: Push to GitLab failed. Aborting."; exit 1 }
    Write-Host "  Pushed to GitLab (gitlab/main)"
}
elseif ($Platform -eq "jenkins") {
    git push origin main
    if ($LASTEXITCODE -ne 0) { Write-Host "ERROR: Push for Jenkins failed. Aborting."; exit 1 }
...
# Capture the inject commit SHA after confirmed push
...
# FIX: Use call operator & so PowerShell correctly splats the array to the external executable
...
if ($Platform -eq "gitlab") {
    git push gitlab main
    if ($LASTEXITCODE -ne 0) { Write-Host "ERROR: Restore push to GitLab failed."; exit 1 }
}
else {
    git push origin main
    if ($LASTEXITCODE -ne 0) { Write-Host "ERROR: Restore push failed."; exit 1 }
}
Write-Host "  Restore committed and pushed."
# Step 6: Summary
Write-Host ""
Write-Host "[6/6] Run complete."
$ResultsPath = "experiment_results\$ExperimentId\$Platform\run_$RunNumber"
Write-Host "  Results saved in: $ResultsPath"
